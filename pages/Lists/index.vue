      ...
      <v-col sm="6" offset-sm="3">
        ...
        <v-treeview :items="lists">
          <template v-slot:append="{item}">
            ...
            <v-icon @click.stop="removeList(item)">mdi-delete-outline</v-icon>
          </template>
        </v-treeview>
...
<script>
  ...
  export default {
    ...
    ,
    methods: {
      ...mapActions('lists', ['getAllLists', 'postNewList', 'patchList']),
      ...mapMutations('lists', ['SET_LIST','ADD_LIST','EDIT_LIST']),
      addItem() {
        const payload = {
          user: 1,
      ...
      editList(item) {
        this.selectedList = {...item}//relleno el campo a editar con el nombre de item que seleccioné
